...
class TestTestCase(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        # 发送请求添加项目
        response2 = requests.request(url=pro_url, method="post", json=pro_data, headers=headers)
        res2 = response2.json()
        # 保存添加的项目id
        ...
        response3 = requests.request(url=inter_url, method="post", json=inter_data, headers=headers)
        res3 = response3.json()
        # 保存接口id
